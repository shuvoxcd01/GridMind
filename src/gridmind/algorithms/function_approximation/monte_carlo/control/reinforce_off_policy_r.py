...
class ReinforceOffPolicy(BaseLearningAlgorithm):
    def __init__(self,  env:Env, 
        ...
        self.behavior_policy = behavior_policy if behavior_policy is not None else RandomPolicy(num_actions=self.num_actions)
    ...
    def _train(self, num_episodes, prediction_only:bool = False):
        ...
        for i in trange(num_episodes):
            collect_episode(env=self.env, policy=self.behavior_policy, trajectory=trajectory, obs_preprocessor=self._preprocess)
            ...
            for timestep in range(trajectory.get_trajectory_length()):
                ...
                behavior_action_prob = self.behavior_policy.get_action_probs(obs, action)
