...
class ReinforceOffPolicyExperience(BaseLearningAlgorithm):
    def __init__(
        ...
        self.behavior_policy = (
            ...
            else EpsilonRandomizedPolicyWrapper(
                policy=self.target_policy, num_actions=self.num_actions, epsilon=0.2
            )
        )
    ...
    def set_policy(
        self, policy: DiscreteActionMLPPolicy, update_behavior_policy: bool = False
    ):
        self.target_policy = policy
        if update_behavior_policy:
            self.behavior_policy = EpsilonRandomizedPolicyWrapper(
                policy=policy, num_actions=self.num_actions, epsilon=0.2
            )
    ...
    def _train_with_trajectory(self, trajectory):
        ...
        for timestep in range(trajectory.get_trajectory_length()):
            ...
            if timestep % 100 == 0 and self.write_summary:
                ...
                self.log_weights_and_grads(
                    self.target_policy, policy_grads, self.global_step, "Target_Policy"
                )

            with torch.no_grad():
                for param, grad in zip(self.value_estimator.parameters(), value_grads):
                    param.copy_(
                        param.data
            ...
            with torch.no_grad():
                for param, grad in zip(self.target_policy.parameters(), policy_grads):
                    param.copy_(
                        param.data
                        + importance_weight
...
if __name__ == "__main__":
    ...
    algorithm = ReinforceOffPolicyExperience(
        env=env, target_policy=target_policy, policy_step_size=0.0001
    )
