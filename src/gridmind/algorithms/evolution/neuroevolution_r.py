    ...
    def train(self, num_generations: int):
        ...
        for generation in trange(num_generations):
            ...
            with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
                fitness_scores, episode_buffers = pool.map(self.evaluate_fitness, [agent.network for agent in agent_to_assess_fitness])

            for agent, fitness, episode_buffer in zip(agent_to_assess_fitness, fitness_scores, episode_buffers):
