...
class ConfigLoader:
    ...
    def load_config(config_file_path) -> dict:
        ...
        if population == "None":
            ...
            raise ValueError(
                f"Unsupported population type: {population}. Expected 'None'."
            )

        policy_network_class = config.get("POPULATION", "policy_network_class")
        policy_network_class = ConfigLoader.POLICY_NETWORK_CLASS_MAP.get(
            policy_network_class, None
        )

        policy_network_creator_fn = config.get(
            "POPULATION", "policy_network_creator_fn"
        )
        if policy_network_creator_fn == "None":
            policy_network_creator_fn = None
        else:
            raise ValueError(
                f"Unsupported policy network creator function: {policy_network_creator_fn}. Expected 'None'."
            )

        if policy_network_class is None and policy_network_creator_fn is None:
            raise ValueError(
                "Either 'policy_network_class' or 'policy_network_creator_fn' must be specified in the configuration."
            )
        ...
        if feature_constructor == "None":
            ...
            raise ValueError(
                f"Unsupported feature constructor: {feature_constructor}. Expected 'None'."
            )
        ...
        parent_selection_fn = ConfigLoader.SELECTION_FN_MAP.get(
        ...
        if stopping_score == "None":
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid stopping score: {stopping_score}. It should be a number or 'None'."
                )
        ...
        replay_buffer_capacity = config.getint(
            "REPLAY_BUFFER", "replay_buffer_capacity"
        )
        replay_buffer_minimum_size = config.getint(
            "REPLAY_BUFFER", "replay_buffer_minimum_size"
        )
        ...
        if q_network == "None":
            ...
            raise ValueError(
                f"Unsupported Q-network type: {q_network}. Expected 'None'."
            )
        q_network_preferred_device = config.get(
            "Q_LEARNING", "q_network_preferred_device"
        )
        if q_network_preferred_device == "None":
            q_network_preferred_device = None
        else:
            raise ValueError(
                f"Unsupported Q-network preferred device: {q_network_preferred_device}. Expected 'None'."
            )
        ...
        q_learner_target_network_update_frequency = config.getint(
            "Q_LEARNING", "q_learner_target_network_update_frequency"
        )
        ...
        num_individuals_to_train_q_fn = config.getint(
            "Q_LEARNING", "num_individuals_to_train_q_fn"
        )
        selection_fn_to_train_q_fn = config.get(
            "Q_LEARNING", "selection_fn_to_train_q_fn"
        )
        selection_fn_to_train_q_fn = ConfigLoader.SELECTION_FN_MAP.get(
            selection_fn_to_train_q_fn, None
        )

        num_top_k = config.getint("SELECTION_AND_EVALUATION", "num_top_k")
        num_elites = config.getint("SELECTION_AND_EVALUATION", "num_elites")
        score_evaluation_num_episodes = config.getint(
            "SELECTION_AND_EVALUATION", "score_evaluation_num_episodes"
        )
        fitness_evaluation_num_samples = config.getint(
            "SELECTION_AND_EVALUATION", "fitness_evaluation_num_samples"
        )
        reevaluate_agent_score = config.getboolean(
            "SELECTION_AND_EVALUATION", "reevaluate_agent_score"
        )
        evaluate_q_derived_policy = config.getboolean(
            "SELECTION_AND_EVALUATION", "evaluate_q_derived_policy"
        )
        ...
        if summary_dir == "None":
            ...
            raise ValueError(
                f"Unsupported summary directory: {summary_dir}. Expected 'None'."
            )
