    ...
    def train(self, num_generations: int):
        ...
        for generation in trange(num_generations):
            ...
            with multiprocessing.Pool(processes=self.num_processes) as pool:
                evaluation_results = pool.map(
                    self.evaluate_fitness,
                    [agent.network for agent in agent_to_assess_fitness],
                )

            for agent, evaluation_result in zip(
                agent_to_assess_fitness, evaluation_results
            ):
                ...
                if self.best_agent is None or agent.fitness > self.best_agent.fitness:
                    ...
                    if (
                        self.highest_fitness_possible is not None
                        and self.best_agent.fitness >= self.highest_fitness_possible
                    ):
...
if __name__ == "__main__":
    ...
    best_policy, best_fitness, trajectories, all_policies = algorithm.train(
